...
class Bot(object):
    ...
    def join_channel(self, channel):
        ...
        self.chat(channel, 'Heippa maailma.')
    ...
    def _oracle(self, question):
        ...
        self.chat(self._channel, answer)
    ...
    def _dailyhoroscope(self, horoscope):
        ...
        if horoscope in horoscopes:
            ...
            self.chat(self._channel, horoscope+answer[0])
        else:
            self.chat(self._channel, 'En ymmärtänyt')
    ...
    def _handle_bangs(self, cmd):
        ...
        if bang in ['quit', 'die', 'part', 'lopeta', 'kuole']:
            self.chat(self._channel, 'Mie meen pois')
            self.quit()

        if bang in ['moi', 'terve', 'hei', 'hi', 'hello']:
            self.chat(self._channel, 'Moro')
